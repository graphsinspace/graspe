...
def hubness_stats(emb_directory, d, out, g_name=None, preset="_", algs=None, k=5):
    ...
    for g_name in datasets:
        ...
        for i in range(emb_fact.num_methods()):
            ...
            recg_f1 = {node: ((2*recg_maps[node]*recg_recalls[node])/(recg_maps[node]+recg_recalls[node]) if recg_maps[node]+recg_recalls[node] != 0 else 0) for node in recg_maps}
            ...
            output.append("Fraction of nodes with low hubness: {}".format(len(map_lo) / g.nodes_cnt()))
            output.append("Fraction of nodes with high hubness: {}".format(len(map_hi) / g.nodes_cnt()))
            ...
            calc_correlations_one_to_many(sets, ("map", recg_maps), output, base_filename)
            calc_correlations_one_to_many(sets, ("recall", recg_recalls), output, base_filename)
...
def calc_correlations_pairwise(sets, output, base_filename):
    ...
    for i in range(len(names)):
        ...
        for j in range(i+1, len(names)):
            name2 = names[j]
            calc_correlations((name1, sets[name1]), (name2, sets[name2]), output, base_filename)    

def calc_correlations_one_to_many(multiple_sets, single_set, output, base_filename):
    for name in multiple_sets:
        calc_correlations((name, multiple_sets[name]), single_set, output, base_filename)
...
def calc_correlations(set1, set2, output, base_filename):
    ...
    output.append("Correlation between {} and {} (pearson, spearman, kendall): {}, {}, {}".format(name1, name2, pearson, spearman, kendall))
...
def nonparametric_tests(name, group_lo, group_hi, output):
    ...
    output.append("Mann Whitney U for {} values of two groups of nodes 1) nodes with low hubness, 2) nodes with high hubness: {}".format(name, pU))
    output.append("Probability that low hubness points have greater {}: {}".format(name, group_lo_ps))
    output.append("Probability that high hubness points have greater {}: {}".format(name, group_hi_ps))
