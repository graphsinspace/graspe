...
def l_2nd(beta):
    def loss_2nd(y_true, y_pred):
        b_ = tf.where(tf.equal(y_true, 0), tf.constant(1, dtype=tf.float32), tf.constant(beta, dtype=tf.float32))
...
class SDNEEmbedding(Embedding):
    ...
    def __init__(self, g, d, hidden_size=[32, 16], alpha=1e-6, beta=5., nu1=1e-5, nu2=1e-4, batch_size=1024, epochs=1, verbose=1):
        """
        Parameters
        ----------
        ...
        encoder_layer_list : list
            A list of numbers of the neuron at each encdoer layer, the last number must be greater than d.        
        """
        super().__init__(g, d)
    ...
    def embed(self):
        ...
        if self.batch_size >= self.node_size:
            if self.batch_size > self.node_size:
                ...
                self.batch_size = self.node_size
            self.model.fit([self.A.todense(), self.L.todense()], [self.A.todense(), self.L.todense()],
            ...
            for epoch in range(initial_epoch, self.epochs):
                ...
                for i in range(steps_per_epoch):
                    index = np.arange(
                        i * self.batch_size, min((i + 1) * self.batch_size, self.node_size))
