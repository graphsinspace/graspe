...
def test_link_pred_unsupervised():
    graph = DatasetPool.load("karate_club_graph")
    h = 20  # broj hidden grana
    k = 20  # parametar k, broj predikcija za selektovanje

    ulp = UnsupervisedLinkPrediction(graph, h, "node2vec")
    patk = ulp.get_precision_at_k(h)
    ulp_map = ulp.get_map(h)

    print("precision@k=", patk)
    print("map=", ulp_map)

    assert ulp_map is not None
    assert patk is not None
