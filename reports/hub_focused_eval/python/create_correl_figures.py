...
d = 10
results = filter(
    lambda r: r.algorithm == algorithm and r.d == d, cmn.get_result_files("..")
)
    ...
    for correl in result.correls:
        for stat in result.correls[correl]:
            overall_chart_data.append(
                [
                    correl.replace("Correlation between", ""),
                    result.dataset,
                    stat,
                    result.correls[correl][stat],
                ]
            )

f_data = pd.DataFrame(
    data=overall_chart_data, columns=["correl", "dataset", "measures", "correlation"]
)
